...
// Render cart: name top, price x qty left, row total right, overall total bottom
function renderCart() {
  ...
  cart.forEach((tree, index) => {
    ...
    item.className = "flex flex-col p-2 border rounded-md";
    item.id = `cart-item-${tree.id}`;
    item.innerHTML = `
      <span class="font-medium text-[#15803d]">${tree.name}</span>
      <div class="flex justify-between items-center mt-1">
        <span class="text-sm text-gray-700">৳ ${tree.price} x ${tree.quantity}</span>
        <span class="text-sm font-bold text-gray-700">৳ ${itemTotal}</span>
        <button class="text-red-500 font-bold ml-2" onclick="removeFromCart(${index})">❌</button>
      </div>
    `;
    cartItemsContainer.appendChild(item);
  });

  // Overall cart total at bottom
